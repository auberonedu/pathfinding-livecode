...
public class PathFinder {
  ...
  public static <T> int distance(Vertex<T> start, Vertex<T> end) {
    ...
    // Map to hold the shortest distance to each vertex
    Map<Vertex<T>, Integer> dists =new HashMap<>();
    Map<Vertex<T>, Vertex<T>> prevs = new HashMap<>(); // to keep track of the path

    minQ.add(new Edge<>(0, start)); // initial edge with distance 0
    ...
    while (!minQ.isEmpty()) {
      ...
      if (dists.containsKey(current.endpoint)) {
        continue; // Skip if we have already found a shorter path to this vertex
      }

      dists.put(current.endpoint, current.weight);

      for (Edge<T> neighbor : current.endpoint.edges) {
        // If the endpoint of the neighbor has not been visited
        if (!dists.containsKey(neighbor.endpoint)) {
          // Calculate the new distance
          int newDistance = current.weight + neighbor.weight;

          Edge<T> newEdge = new Edge<>(newDistance, neighbor.endpoint); // create a new edge with the updated distance
          minQ.add(newEdge); // add the new edge to the priority queue
          if (!prevs.containsKey(neighbor.endpoint)) {
            prevs.put(neighbor.endpoint, current.endpoint); // update the previous vertex for path reconstruction
          }
        }
      }
    }
    ...
    // Reconstruct the path from end to start using the prevs map
    while(current != null) {
    ...
    return dists.get(end); // return the shortest distance to the end vertex
  }
}
