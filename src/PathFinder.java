...
public class PathFinder {
  ...
  public static <T> int distance(Vertex<T> start, Vertex<T> end) {
    ...
    while(!minQ.isEmpty()){
      ...
      for(var edge : current.endpoint.edges){
        if(!dists.containsKey(edge.endpoint)){
          int newDistance = current.weight + edge.weight;
          Edge<T> newEdge = new Edge<>(newDistance, edge.endpoint);
          minQ.add(newEdge);
          if(!prev.containsKey(edge.endpoint)){
            prev.put(edge.endpoint, current.endpoint);
          }
        }
      }
    }
